...
async function startEc2Instance(label, githubRegistrationToken) {
  ...
  const userData = [
    ...
    `./config.sh --unattended --name ${label} --labels ${label} --replace --url https://github.com/${config.githubContext.owner}/${config.githubContext.repo} --token ${githubRegistrationToken}`,
    './run.sh',
  ];
